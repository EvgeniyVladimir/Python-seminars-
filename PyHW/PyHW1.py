# Задача: Напишите программу, которая принимает на вход цифру, обозначающую день недели, и проверяет, является ли этот день выходным.
# Пример:
# - 6 -> да
# - 7 -> да
# - 1 -> нет

# a = int(input('введите число дня недели '))
# week = [1, 2, 3, 4, 5, 6, 7]
# if a < week[5]:
#     print('нет')
# else:
#     print('да')

# Задача: Напишите программу для. проверки истинности утверждения ¬(X ⋁ Y ⋁ Z) = ¬X ⋀ ¬Y ⋀ ¬Z для всех значений предикат.

# for x in range(2):
#     for y in range(2):
#         for z in range(2):
#             print(not (x or y or z) == (not x and not y and not z))
#             print(x, y, z)

# Задача: Напишите программу, которая принимает на вход координаты точки (X и Y), причём X ≠ 0 и Y ≠ 0 и выдаёт номер четверти плоскости, в которой находится эта точка (или на какой оси она находится).

# Пример:
# - x=34; y=-30 -> 4
# - x=2; y=4-> 1
# - x=-34; y=-30 -> 3

# x = int(input('Введите координату по оси X: '))
# y = int(input('Введите координату по оси Y: '))
# if x==0 or y==0:
#     print('Введите координаты X ≠ 0 и Y ≠ 0')
# elif x>0 and y>0:
#     print('1 четверть')
# elif x<0 and y>0:
#     print('2 четверть')
# elif x<0 and y<0:
#     print('3 четверть')
# else:
#     print('4 четверть')


# Задача: Напишите программу, которая по заданному номеру четверти, 
# показывает диапазон возможных координат точек в этой четверти (x и y).

# x = int(input('Введите номер четверити от 1 до 4: '))
# if x <= 0 or x > 4:
#     print('Введите номер четверити от 1 до 4: ')
# elif x==1:
#     print('координаты точек: x > 0 and y > 0')
# elif x==2:
#     print('координаты точек: x < 0 and y > 0')
# elif x==3:
#     print('координаты точек: x < 0 and y < 0')
# else:
#     print('координаты точек: x > 0 and y < 0')

# Задача: Напишите программу, которая принимает на вход координаты двух точек и находит расстояние между ними в 2D пространстве.
# Пример:

# - A (3,6); B (2,1) -> 5,09
# - A (7,-5); B (1,-1) -> 7,21

# xa = int(input('Введите координату X первой точки: '))
# ya = int(input('Введите координату Y первой точки: '))
# xb = int(input('Введите координату X второй точки: '))
# yb = int(input('Введите координату Y второй точки: '))
# result = ((xb - xa)**2 + (yb - ya)**2)**0.5
# print(round(result, 2))

